...
let calculatePosition = (ref, eye, e, setState) => {
    ...
    const divisor = 500 //  divisor мб переназвати
...
const MovingEyes = ({ eyesClassName }) => {
    ...
    useEffect(() => {
        const handleMouseMove = (e) => {
            if (leftPupil.current) calculatePosition(leftPupil, eye, e, setLeftPupilTransform)
            if (rightPupil.current) calculatePosition(rightPupil, eye, e, setRightPupilTransform)
        };
    ...
    return <div className={`eyes ${eyesClassName}`} >
        {eyesElements}
    </div>
